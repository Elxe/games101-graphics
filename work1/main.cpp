...
Eigen::Matrix4f get_model_matrix(float rotation_angle)
{
    ...
    // Eigen::AngleAxisf rv(rotation_angle, Eigen::Vector3f(0.0, 0.0, 1.0));
    // Eigen::Matrix3f rm =  rv.matrix();
    // for (int i = 0; i < 3; i++) {
    //     for (int j = 0; j < 3; j++) {
    //         model(i,j) = rm(i,j);
    //     }
...
Eigen::Matrix4f get_rotation(const Eigen::Vector3f& axis, float angle) {
    ...
    Eigen::AngleAxisf rv(angle, axis);
    Eigen::Matrix3f rm =  rv.matrix();
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            model(i,j) = rm(i,j);
...
int main(int argc, const char **argv)
{
    ...
    while (key != 27)
    {
        ...
        // r.set_model(get_model_matrix(angle / 180 * M_PI));
        r.set_model(get_rotation(Eigen::Vector3f(0.0, 1.0, 0.0), angle / 180 * M_PI));
        r.set_view(get_view_matrix(eye_pos));
        r.set_projection(get_projection_matrix(45, 1, 0.1, 50));
