...
{   
    ...
    Eigen::Vector3f P(0.5+static_cast<float>(x), 0.5+static_cast<float>(y), 0.0);
    Eigen::Vector3f AB = _v[1] - _v[0];
    Eigen::Vector3f BC = _v[2] - _v[1];
    Eigen::Vector3f CA = _v[0] - _v[2];
    Eigen::Vector3f AP = P - _v[0];
...
int min(const std::vector<float>& vec) {
    return std::floor(*std::min_element(vec.begin(), vec.end()));
}

int max(const std::vector<float>& vec) {
    return std::ceil(*std::max_element(vec.begin(), vec.end()));
}

float cross(const Eigen::Vector2f v1, const Eigen::Vector2f v2) {
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    Tri[0] = Eigen::Vector3f(v[0].x(), v[0].y(), 0.0);
    Tri[1] = Eigen::Vector3f(v[1].x(), v[1].y(), 0.0);
    Tri[2] = Eigen::Vector3f(v[2].x(), v[2].y(), 0.0);
    
    // TODO : Find out the bounding box of current triangle.
    // iterate through the pixel and find if the current pixel is inside the triangle
    Eigen::Vector2d lt(min({Tri[0].x(), Tri[1].x(), Tri[2].x()}), min({Tri[0].y(), Tri[1].y(), Tri[2].y()}));
